...
3. **Copier la valeur immédiatement** — elle ne sera plus jamais affichée.
   C'est ce secret qui ira dans le *secret scope* Databricks, et **nulle part
   ailleurs**.
...
| Sujet | À faire |
|---|---|
| **Expiration du secret client** | Noter la date dans l'agenda. À expiration, le job échoue avec « Entra ID refusée » — sans préavis. Renouveler dans Entra ID puis mettre à jour le secret scope. |
