...
from bl_core import (extraction, notifications, pdf_bl, rapports, rbac,
                     repository, scan_pdf, ui)
...
NAVIGATION = [
    ...
    ("Achat", [("📥", "BL réception"), ("📡", "DESADV achat"),
               ("⚖️", "Rapprochement achat"), ("🧾", "Scans à valider (achat)")]),
    ("Vente", [("📤", "BL expédition"), ("📡", "DESADV vente"),
               ("⚖️", "Rapprochement vente"), ("🧾", "Scans à valider (vente)")]),
    ("Pipeline", [("📥", "Scans reçus")]),
...
LIBELLES_VUES = {"Rapprochement achat": "Rapprochement BL / DESADV",
                 ...
                 "Scans à valider (achat)": "Scans à valider",
                 "Scans à valider (vente)": "Scans à valider"}
# Vues qui exploitent toute la largeur : le fil d'Ariane y est remplacé par un
# titre compact, pour laisser la hauteur d'écran à l'image en relecture.
VUES_PLEIN_ECRAN = {"Scans à valider (achat)", "Scans à valider (vente)"}

# Colonne visée par le filtre de période des vues BL.
LIBELLES_CHAMP_DATE = ["📅 Date d'opération", "🖊️ Date de saisie"]
CHAMPS_DATE_PAR_LIBELLE = {
    LIBELLES_CHAMP_DATE[0]: repository.CHAMP_DATE_OPERATION,
    LIBELLES_CHAMP_DATE[1]: repository.CHAMP_DATE_SAISIE,
}
...
# --- Validation des scans : un seul scan ouvert à la fois ------------------
def _fermer_scan_ouvert() -> None:
    """Quitter la vue libère le verrou : un scan pris par erreur ne reste pas
    bloqué pour les autres gestionnaires."""
    ouvert = st.session_state.pop("scan_ouvert", None)
    st.session_state.pop("scan_pages_cache", None)
    if ouvert is not None:
        try:
            repository.liberer_scan(int(ouvert))
        except Exception:
            logger.warning("Libération du scan %s impossible", ouvert)
    ...
    for section, vues in NAV_VISIBLE:
        ...
        for icone, v in vues:
            ...
            if st.button(f"{icone}  {libelle}", key=f"nav_{v}", width="stretch",
                ...
                # Quitter l'archivage libère les threads d'analyse en cours.
                if v not in VUES_PLEIN_ECRAN:
                    _fermer_scan_ouvert()
                st.rerun()
...
PLEIN_ECRAN = vue in VUES_PLEIN_ECRAN
# Titre et contexte sur une seule ligne, y compris sur les vues plein écran :
# l'en-tête tient désormais sur une bande, la place gagnée reste à l'image.
ui.entete_app("Administration des BL", utilisateur=utilisateur,
              environnement=SETTINGS.environment, roles=CTX_RBAC["roles"])
ui.show_flash()
...
def render_dashboard() -> None:
    ...
    with col_top:
        ...
        # Lecture : « quelle part du problème EDI ce tiers représente-t-il ? »
        # (et non « quel est son taux d'échec ? »). Un tiers à 100 % d'échec
        # sur 2 flux ne doit pas masquer celui qui concentre 300 NOK.
        st.markdown(f"#### Top 10 {tiers_dashboard} (part des EDI NOK)")
        ...
        if not par_frs:
            ...
            total_nok = int(cumul["nok"].sum())
            if not total_nok:
                st.success("Aucun NOK sur la période : rien à signaler. ✅")
            else:
                # Part du tiers dans le TOTAL des NOK du périmètre filtré.
                cumul["Part"] = (100 * cumul["nok"] / total_nok).round(1)
                pires = (cumul[cumul["nok"] > 0]
                         .sort_values(["nok", "total"], ascending=False)
                         .head(10).rename_axis("Fournisseur").reset_index())
                barres = (
                    alt.Chart(pires).mark_bar(color="#E4572E", cornerRadiusEnd=3).encode(
                        x=alt.X("Part:Q", title=f"% des {total_nok} EDI NOK du périmètre"),
                        y=alt.Y("Fournisseur:N", sort="-x", title=None,
                                axis=alt.Axis(labelFontSize=12, labelLimit=280)),
                        tooltip=["Fournisseur",
                                 alt.Tooltip("Part:Q", title="Part des NOK (%)",
                                             format=".1f"),
                                 alt.Tooltip("nok:Q", title="NOK"),
                                 alt.Tooltip("total:Q", title="Flux contrôlés")],
                    ).properties(height=290)
                )
                st.altair_chart(barres, use_container_width=True)
                st.caption(
                    f"Les 10 premiers concentrent "
                    f"{pires['Part'].sum():.0f} % des {total_nok} EDI NOK de la "
                    "période et du périmètre filtrés.")
...
def vue_bl(nom_vue: str, types: list[str]) -> None:
    ...
    with st.expander("🔍 Filtres", expanded=False):
        # La période s'applique au choix à la date métier ou à la date
        # d'enregistrement : un archivage saisi hier pour un BL de 2024 est
        # introuvable par la seule date d'opération.
        libelle_champ = st.segmented_control(
            "La période porte sur", LIBELLES_CHAMP_DATE,
            default=LIBELLES_CHAMP_DATE[0], key=f"f_champdate_{nom_vue}")
        champ_date = CHAMPS_DATE_PAR_LIBELLE.get(
            libelle_champ, repository.CHAMP_DATE_OPERATION)
    ...
    if sel_per and champ_date == repository.CHAMP_DATE_SAISIE:
        chips.append(("🖊️ Période sur la date de saisie",
                      f"f_champdate_{nom_vue}", LIBELLES_CHAMP_DATE[0]))
    if f_numero:
    ...
    signature = (f_numero, f_tiers, str(dmin), str(dmax), f_statut, f_gest,
                 f_suppr, champ_date)
    ...
    try:
        ...
        stats = repository.stats_bl(numero=f_numero, fournisseur=f_tiers, types=types,
                                    ...
                                    inclure_supprimes=f_suppr, champ_date=champ_date)
    except Exception as e:
        st.error(f"Erreur de lecture de la base : {e}")
        st.stop()
    ...
    with ruban:
        ...
        # Restaurer n'a de sens que sur des BL effectivement supprimés :
        # sélection non vide ET entièrement composée de lignes supprimées.
        selection = df[df["id_bl"].isin(ids_selection)] if n else df.iloc[0:0]
        tous_supprimes = bool(n) and bool(
            selection["est_supprime"].fillna(False).astype(bool).all())
        if not lecture:
            specs.insert(1, ("✏️ Modifier", "mod", n != 1, 1.3))
            if avec_statut:
                specs.append(("✅ Passer à OK", "ok", n == 0, 1.5))
            specs += [("🗑️ Supprimer", "sup", n == 0, 1.4),
                      ("♻️ Restaurer", "res", not tous_supprimes, 1.4)]
        ...
        for i, (label, code, disabled, _) in enumerate(specs):
            aide = ("Cliquez sur une ligne pour la sélectionner."
                    ...
                    else "Passe les BL EDI NOK sélectionnés à OK." if code == "ok"
                    else "Sélectionnez uniquement des BL supprimés." if code == "res"
                    else None)
...
# =====================================================================
# VUE « SCANS À VALIDER » — un scan = un BL multi-pages
# =====================================================================
# Le pipeline (jobs/traitement_scans.py) a déjà analysé chaque scan et n'a
# renvoyé ici que ceux dont le rapprochement était insuffisant. Le
# gestionnaire corrige les champs pré-remplis et crée le BL — ou écarte le
# scan. L'analyse n'est PAS refaite à l'ouverture : elle est lue depuis la
# colonne `extraction`, ce qui rend l'écran instantané et sans coût.
def _scan_referentiel(type_tiers: str, sens: str):
    """Même contexte de rapprochement que le job : liste des tiers, adresses
    de sites, numéros de BL connus par tiers."""
    try:
        tous_tiers = repository.lister_tiers(type_tiers)
    except Exception as exc:
        st.error(f"Référentiel des tiers indisponible : {exc}")
        tous_tiers = []
    try:
        adresses = repository.adresses_par_tiers()
    except Exception:
        adresses = {}
    return tous_tiers, extraction.Referentiel(
        tiers=tous_tiers,
        bls_pour_tiers=lambda nom: repository.bls_desadv_pour_tiers(nom, sens),
        adresses=adresses)


def _scan_pages(scan_id: int, pdf: bytes) -> list[bytes]:
    """Pages rasterisées du scan, mémorisées pour la durée de la session : on
    ne relit le PDF qu'une fois par scan ouvert."""
    cache = st.session_state.setdefault("scan_pages_cache", {})
    if scan_id not in cache:
        cache.clear()                  # un seul scan ouvert à la fois
        cache[scan_id] = scan_pdf.rasteriser(pdf)
    return cache[scan_id]


@st.dialog("🗑️ Écarter ce scan", width="medium")
def dialog_ecarter_scan(scan_id: int, nom_fichier: str):
    st.caption(f"Scan « {nom_fichier} ». Le PDF et sa trace sont conservés ; "
               "aucun BL ne sera créé.")
    motif = st.text_area("Motif (obligatoire)", max_chars=500,
                         placeholder="Ex. : ce n'est pas un bordereau / "
                                     "doublon d'un scan déjà traité").strip()
    if st.button("Confirmer l'écartement", type="primary", width="stretch",
                 disabled=not motif):
        repository.cloturer_scan(scan_id, repository.STATUT_SCAN_REJETE,
                                 motif=motif, traite_par=utilisateur)
        st.session_state.pop("scan_ouvert", None)
        ui.set_flash("toast", "Scan écarté")
        st.rerun()


def vue_validation_scans(sens: str) -> None:
    ...
    tiers_libelle = repository.libelle_tiers(type_operation)

    try:
        df = repository.lister_scans(
            statuts=[repository.STATUT_SCAN_A_VALIDER], sens=sens)
    except Exception as exc:
        st.error(f"Erreur de lecture : {exc} (migration V001 exécutée ?)")
        st.stop()

    ouvert = st.session_state.get("scan_ouvert")
    ...
    if not ouvert:
        st.caption(
            "Scans que le pipeline n'a pas pu rapprocher avec certitude. Les "
            "champs sont pré-remplis par l'IA : corrigez ce qui doit l'être, "
            "puis créez le BL. Les scans rapprochés avec un avis d'expédition "
            f"sont déjà devenus des BL, visibles dans « BL "
            f"{'expédition' if est_vente else 'réception'} ».")
        if st.button("🔄 Actualiser", key=f"maj_scans_{sens}"):
            st.rerun()
        if df is None or df.empty:
            st.success("✅ Aucun scan en attente de validation.")
            return
        st.metric("Scans à valider", len(df))
        selection = st.dataframe(
            df[["recu_le", "nom_fichier", "nb_pages", "confiance",
                "motif_validation", "verrouille_par"]],
            hide_index=True, width="stretch", height=min(430, 60 + 36 * len(df)),
            on_select="rerun", selection_mode="single-row",
            key=f"grille_scans_{sens}",
            column_config={
                "recu_le": st.column_config.DatetimeColumn(
                    "Reçu le", format="DD/MM/YYYY HH:mm"),
                "nom_fichier": st.column_config.TextColumn("Fichier", width="medium"),
                "nb_pages": st.column_config.NumberColumn("Pages"),
                "confiance": st.column_config.TextColumn("Confiance"),
                "motif_validation": st.column_config.TextColumn(
                    "Pourquoi une validation", width="large"),
                "verrouille_par": st.column_config.TextColumn("Pris par"),
            })
        lignes = (selection.selection.rows
                  if selection and selection.selection else [])
        if not lignes:
            st.caption("Sélectionnez une ligne pour ouvrir le scan.")
            return
        scan = df.iloc[lignes[0]]
        actions = st.columns([2, 2, 4])
        if actions[0].button("📖 Prendre en charge", type="primary",
                             width="stretch", disabled=LECTURE_SEULE):
            if repository.verrouiller_scan(int(scan["id"]), utilisateur,
                                           SETTINGS.scan_verrou_minutes):
                st.session_state.scan_ouvert = int(scan["id"])
                st.rerun()
            else:
                st.warning(f"Ce scan est déjà pris en charge par "
                           f"{scan['verrouille_par']}. Réessayez plus tard, ou "
                           "attendez la libération automatique du verrou.")
        if actions[1].button("🗑️ Écarter", width="stretch",
                             disabled=LECTURE_SEULE):
            dialog_ecarter_scan(int(scan["id"]), scan["nom_fichier"])
        return

    # -----------------------------------------------------------------
    # Écran de validation d'UN scan
    # -----------------------------------------------------------------
    detail = repository.telecharger_scan(ouvert)
    if detail is None or detail["statut"] != repository.STATUT_SCAN_A_VALIDER:
        st.session_state.pop("scan_ouvert", None)
        st.info("Ce scan n'est plus à valider (déjà traité par ailleurs).")
        st.rerun()

    pages = _scan_pages(ouvert, detail["contenu"])
    tous_tiers, referentiel = _scan_referentiel(type_tiers, sens)
    infos = detail.get("extraction") or {}

    gauche, droite = st.columns([1, 2.3], gap="medium")

    with gauche:
        st.markdown(f"#### Scan « {detail['nom_fichier']} »")
        st.caption(f"Reçu le {detail['recu_le']:%d/%m/%Y à %H:%M} de "
                   f"{detail['expediteur'] or '—'} · {len(pages)} page(s)")
        if detail.get("motif_validation"):
            st.warning(f"**Pourquoi cette validation :** "
                       f"{detail['motif_validation']}")
        if infos.get("ecart_tiers"):
            st.info(f"ℹ️ {infos['ecart_tiers']}")

        if st.button("🔄 Relancer l'analyse IA", width="stretch",
                     help="Utile après avoir corrigé le référentiel des tiers."):
            with st.spinner("Analyse des pages du bordereau…"):
                try:
                    frais = extraction.extraire_infos_bl(
                        pages, tiers_libelle.lower(), referentiel)
                    repository.cloturer_scan(
                        ouvert, repository.STATUT_SCAN_A_VALIDER,
                        extraction_json={**infos, **frais},
                        motif=detail.get("motif_validation") or "")
                except Exception as exc:
                    st.error(f"Analyse impossible : {exc}")
                else:
                    st.rerun()

        numero = st.text_input("Numéro du BL *", max_chars=60,
                               value=str(infos.get("numero_bl") or ""),
                               key=f"scan_num_{ouvert}")
        if infos.get("numero_bl"):
            st.caption(f"🔎 détecté : « {infos['numero_bl']} »")

        date_defaut = _parse_date_scan(infos.get("date_reception")) or \
            _parse_date_scan(infos.get("date")) or detail["recu_le"].date()
        date_bl = st.date_input(
            "Date d'expédition *" if est_vente else "Date de réception *",
            value=date_defaut, key=f"scan_date_{ouvert}")

        reconnu, fiabilite = extraction.rapprocher_tiers(
            infos.get("code_tiers"), infos.get("tiers"), tous_tiers)
        filtre = st.text_input("Filtrer la liste", value="",
                               key=f"scan_filtre_{ouvert}",
                               placeholder="Tapez quelques lettres pour filtrer…")
        options = ([t for t in tous_tiers if filtre.strip().lower() in t.lower()]
                   if filtre.strip() else tous_tiers)
        index_tiers = options.index(reconnu) if reconnu in options else None
        fournisseur = st.selectbox(f"{tiers_libelle} *", options=options,
                                   index=index_tiers, placeholder="Choisir…",
                                   key=f"scan_tiers_{ouvert}")
        detecte = " / ".join(x for x in (infos.get("code_tiers"),
                                        infos.get("tiers")) if x)
        if detecte:
            marque = ("code ✓" if fiabilite == "code"
                      else "nom ✓" if fiabilite == "nom" else "")
            st.caption(f"🔎 détecté : « {detecte} »"
                       + (f" — reconnu ({marque}) : {reconnu}" if reconnu
                          else " — non trouvé au référentiel"))

        # Plage horaire et quai : mêmes règles que la création automatique.
        plage_defaut = str(infos.get("plage_horaire") or "")
        index_plage = (repository.PLAGES_HORAIRES.index(plage_defaut)
                       if plage_defaut in repository.PLAGES_HORAIRES else None)
        plage = st.selectbox("Plage horaire *", repository.PLAGES_HORAIRES,
                             index=index_plage, placeholder="Choisir…",
                             key=f"scan_plage_{ouvert}")
        quais = repository.lister_quais()
        quai_pla = repository.quai_pla(fournisseur) if fournisseur else None
        pre_quai = (str(infos.get("quai") or "") or quai_pla
                    or repository.QUAI_DEFAUT)
        index_quai = quais.index(pre_quai) if pre_quai in quais else None
        quai = st.selectbox("Quai *", quais, index=index_quai,
                            placeholder="Choisir…",
                            key=f"scan_quai_{ouvert}_{fournisseur or ''}")
        if quai_pla:
            st.caption(f"📋 Quai du protocole logistique (PLA) : {quai_pla}")

        if type_operation == repository.TYPE_RECEPTION:
            defaut_nok = str(infos.get("statut_bl") or "1") == "0"
            choix = st.radio("État de réception *", ["OK", "EDI NOK"],
                             index=1 if defaut_nok else 0, horizontal=True,
                             key=f"scan_statut_{ouvert}")
            statut_bl = (repository.STATUT_EDI_NOK if choix == "EDI NOK"
                         else repository.STATUT_OK)
            if infos.get("statut"):
                st.caption(f"🔎 détecté (manuscrit) : « {infos['statut']} »")
        else:
            statut_bl = repository.STATUT_OK
        commentaire = st.text_area("Commentaire (facultatif)", max_chars=1000,
                                   value=str(infos.get("commentaire") or ""),
                                   key=f"scan_comm_{ouvert}")

    with droite:
        # Visionneuse client : zoom, ajustement et rotation sans rerun, sur
        # TOUTES les pages du bordereau.
        ui.visionneuse_images(
            pages, f"Scan {detail['nom_fichier']}", hauteur=780,
            etiquettes=[f"Page {i + 1} / {len(pages)}" for i in range(len(pages))],
            avec_impression=False)

    st.divider()
    anomalie = scan_pdf.controler_bl(
        numero, fournisseur or "",
        lambda num: repository.numero_bl_disponible(num, type_operation))
    if anomalie:
        st.error(f"⛔ {anomalie}")

    actions = st.columns([2, 1.6, 1.6, 3])
    if actions[0].button("✅ Créer le BL", type="primary", width="stretch",
                         disabled=bool(anomalie) or LECTURE_SEULE):
        rbac.exiger_vue(vue, CTX_RBAC, rbac.MODIFICATION)
        id_bl = str(uuid.uuid4())
        try:
            repository.creer_bl_depuis_scan(
                id_bl=id_bl, numero_bl=numero.strip(),
                nom_fournisseur=fournisseur, statut_bl=statut_bl,
                type_operation=type_operation, utilisateur=utilisateur,
                pages=pages, origine="SCAN_VALIDE", date_reception=date_bl,
                quai_reception=quai, comment_bl=commentaire.strip(),
                plage_horaire=plage)
        except Exception as exc:
            st.error(f"Création impossible : {exc}")
        else:
            repository.cloturer_scan(
                ouvert, repository.STATUT_SCAN_VALIDE, id_bl=id_bl,
                traite_par=utilisateur, extraction_json=infos)
            if type_operation == repository.TYPE_RECEPTION:
                _notifier_nouvelle_reception(numero.strip(), fournisseur, quai,
                                             date_bl, plage, statut_bl,
                                             len(pages))
            st.session_state.pop("scan_ouvert", None)
            st.session_state.pop("scan_pages_cache", None)
            ui.set_flash("toast", f"BL {numero.strip()} créé")
            st.rerun()
    if actions[1].button("🗑️ Écarter", width="stretch"):
        dialog_ecarter_scan(ouvert, detail["nom_fichier"])
    if actions[2].button("↩️ Rendre à la file", width="stretch",
                         help="Libère le scan pour un autre gestionnaire."):
        repository.liberer_scan(ouvert)
        st.session_state.pop("scan_ouvert", None)
        st.session_state.pop("scan_pages_cache", None)
        st.rerun()


def _notifier_nouvelle_reception(numero, fournisseur, quai, date_reception,
                                 plage, statut_bl, nb_pages) -> None:
    """Notification Teams après validation humaine — best effort, jamais
    bloquante : le BL est créé dans tous les cas."""
    succes, detail = notifications.notifier_nouvelle_reception(
        numero_bl=numero, fournisseur=fournisseur, quai=quai,
        date_reception=date_reception, plage_horaire=plage,
        statut_libelle=ui.libelle_statut(statut_bl), nb_pages=nb_pages,
        utilisateur=utilisateur)
    if not succes:
        st.warning(f"BL créé, mais la notification Teams n'a pas été publiée "
                   f"({detail}). Elle reste tracée dans Gestion ▸ Notifications.")


# =====================================================================
# VUE « SCANS REÇUS » — supervision du pipeline
# =====================================================================
@st.dialog("📄 Déposer un PDF manuellement", width="medium")
def dialog_depot_manuel():
    """Filet de sécurité : si la chaîne copieur → mail → Graph est en panne,
    un gestionnaire peut injecter un scan à la main. Le PDF rejoint la même
    file et suivra le même traitement."""
    st.caption("Le PDF doit contenir **un seul bordereau** (1 à "
               f"{SETTINGS.scan_max_pages} pages). Il rejoindra la file et "
               "sera analysé par le prochain passage du job de traitement.")
    sens_choisi = st.radio("Sens", ["Achat (réception)", "Vente (expédition)"],
                           horizontal=True)
    fichier = st.file_uploader("Fichier PDF", type=["pdf"])
    if fichier is not None and st.button("📥 Déposer dans la file",
                                         type="primary", width="stretch"):
        octets = fichier.getvalue()
        if not octets.startswith(b"%PDF-"):
            st.error("Ce fichier n'est pas un PDF valide.")
            return
        try:
            nb_pages = scan_pdf.compter_pages(octets)
        except Exception as exc:
            st.error(f"PDF illisible : {exc}")
            return
        if nb_pages > SETTINGS.scan_max_pages:
            st.error(f"{nb_pages} pages : au-delà de la limite de "
                     f"{SETTINGS.scan_max_pages} pour un bordereau.")
            return
        sens = (repository.SENS_VENTE if sens_choisi.startswith("Vente")
                else repository.SENS_ACHAT)
        # Clé d'idempotence dérivée du contenu : redéposer le même fichier ne
        # crée pas de doublon.
        empreinte = hashlib.sha256(octets).hexdigest()[:32]
        identifiant = repository.enregistrer_scan(
            message_id=f"manuel:{empreinte}", source="MANUEL",
            expediteur=utilisateur, destinataire="(dépôt manuel)",
            objet=f"Dépôt manuel — {fichier.name}", nom_fichier=fichier.name,
            contenu=octets, nb_pages=nb_pages, sens=sens,
            recu_le=repository.maintenant_local())
        if identifiant is None:
            st.warning("Ce PDF est déjà dans la file (contenu identique).")
        else:
            ui.set_flash("toast", f"Scan déposé ({nb_pages} page(s))")
            st.rerun()


def vue_scans_recus() -> None:
    st.caption("Journal du pipeline : chaque PDF reçu du copieur, son analyse "
               "et son issue. Sert au diagnostic — les scans à traiter sont "
               "dans « Scans à valider » de chaque module.")

    barre = st.columns([3, 1.4])
    with barre[0]:
        statuts = st.pills(
            "Statut", options=["RECU", "EN_ANALYSE", "TRAITE_AUTO", "A_VALIDER",
                               "VALIDE", "REJETE", "ERREUR"],
            selection_mode="multi", default=[], key="scans_statuts")
    with barre[1]:
        if not LECTURE_SEULE and st.button("📄 Dépôt manuel", width="stretch"):
            dialog_depot_manuel()
    dmin, dmax, _sel = filtre_periode("scans", libelle="Reçus", defaut=())

    try:
        df = repository.lister_scans(statuts=list(statuts) or None,
                                     date_min=dmin, date_max=dmax)
        compteurs = repository.stats_scans(
            (dmin or datetime.date.today() - datetime.timedelta(days=30)))
        dernier = repository.dernier_scan_recu()
    except Exception as exc:
        st.error(f"Erreur de lecture : {exc} (migration V001 exécutée ?)")
        st.stop()

    k = st.columns(5)
    k[0].metric("Reçus", sum(compteurs.values()))
    k[1].metric("BL créés automatiquement", compteurs.get("TRAITE_AUTO", 0))
    k[2].metric("À valider", compteurs.get("A_VALIDER", 0),
                delta_color="inverse" if compteurs.get("A_VALIDER") else "off")
    k[3].metric("En erreur", compteurs.get("ERREUR", 0),
                delta_color="inverse" if compteurs.get("ERREUR") else "off")
    # Un pipeline muet est une panne plus dangereuse qu'une erreur bruyante.
    if dernier is not None:
        heures = (repository.maintenant_local() - dernier).total_seconds() / 3600
        k[4].metric("Dernier scan", f"{heures:.0f} h",
                    help=f"Reçu le {dernier:%d/%m/%Y à %H:%M}",
                    delta_color="inverse" if heures > 8 else "off")
        if heures > 8:
            st.warning(f"⚠️ Aucun scan reçu depuis {heures:.0f} heures. "
                       "Vérifiez le copieur, la boîte de scan et le job "
                       "« ingestion_scans ».")
    else:
        k[4].metric("Dernier scan", "—")

    total = sum(compteurs.values())
    if total:
        auto = compteurs.get("TRAITE_AUTO", 0)
        st.caption(f"Taux d'automatisation sur la période : "
                   f"**{100 * auto / total:.0f} %** ({auto} BL créés sans "
                   f"intervention sur {total} scans).")

    if df is None or df.empty:
        st.info("Aucun scan pour ces critères.")
        return
    st.dataframe(
        df[["recu_le", "sens", "nom_fichier", "nb_pages", "statut", "confiance",
            "motif_validation", "expediteur", "traite_le", "traite_par",
            "tentatives", "erreur"]],
        hide_index=True, width="stretch", height=460,
        column_config={
            "recu_le": st.column_config.DatetimeColumn(
                "Reçu le", format="DD/MM/YYYY HH:mm"),
            "sens": st.column_config.TextColumn("Sens"),
            "nom_fichier": st.column_config.TextColumn("Fichier", width="medium"),
            "nb_pages": st.column_config.NumberColumn("Pages"),
            "statut": st.column_config.TextColumn("Statut"),
            "confiance": st.column_config.TextColumn("Confiance"),
            "motif_validation": st.column_config.TextColumn("Motif", width="large"),
            "expediteur": st.column_config.TextColumn("Expéditeur"),
            "traite_le": st.column_config.DatetimeColumn(
                "Traité le", format="DD/MM/YYYY HH:mm"),
            "traite_par": st.column_config.TextColumn("Par"),
            "tentatives": st.column_config.NumberColumn("Essais"),
            "erreur": st.column_config.TextColumn("Erreur", width="medium"),
        })


def _parse_date_scan(valeur):
    if isinstance(valeur, datetime.date):
        return valeur
    try:
        return datetime.date.fromisoformat(str(valeur or "")[:10])
    except Exception:
        return None
...
def render_vue_courante() -> None:
    if PLEIN_ECRAN:
        st.markdown(f"#### 🧾 Scans à valider · {section.lower()}")
    elif vue != VUE_DASHBOARD:
    ...
    elif vue == "Scans à valider (achat)":
        vue_validation_scans(repository.SENS_ACHAT)
    elif vue == "Scans à valider (vente)":
        vue_validation_scans(repository.SENS_VENTE)
    elif vue == "Scans reçus":
        vue_scans_recus()
