...
def libelle_statut(statut_bl: str) -> str:
    """Libellé lisible de l'état EDI d'un BL.

    Vit ici, à côté de la constante qu'il traduit, et non dans `ui` : les
    tâches Lakeflow en ont besoin pour les notifications, et `ui` importe
    Streamlit — absent de l'environnement d'un job. `ui.libelle_statut` reste
    disponible pour l'application, par réexport."""
    return "✅ OK" if statut_bl == STATUT_OK else "🟥 EDI NOK"
...
# Colonne sur laquelle porte le filtre de période des vues BL.
CHAMP_DATE_OPERATION = "date_reception"   # date métier (réception/expédition)
CHAMP_DATE_SAISIE = "saisie_le"           # date d'enregistrement dans l'outil
CHAMPS_DATE = (CHAMP_DATE_OPERATION, CHAMP_DATE_SAISIE)
...
def roles_utilisateur(utilisateur: str) -> list[str]:
    ...
    **Volontairement NON mise en cache.** Chaque application tourne dans son
    propre processus, avec son propre cache : un cache ici rendait un retrait
    de droit invisible de l'app Création tant que l'entrée n'expirait pas,
    alors que l'app Administration, qui vide le sien après modification, le
    voyait immédiatement. Une décision d'autorisation ne doit jamais être
    servie depuis un état périmé. Le coût est d'une lecture indexée sur une
    table minuscule, une fois par rendu."""
    s = get_settings()
    df = _run(
...
def vider_caches_referentiels() -> None:
    ...
    quai_pla.clear()
    # roles_utilisateur n'est pas mis en cache (autorisation toujours fraîche).
...
# ---------------------------------------------------------------------------
# Scans reçus du copieur — file d'attente du pipeline
# ---------------------------------------------------------------------------
STATUT_SCAN_RECU = "RECU"
STATUT_SCAN_EN_ANALYSE = "EN_ANALYSE"
STATUT_SCAN_AUTO = "TRAITE_AUTO"
STATUT_SCAN_A_VALIDER = "A_VALIDER"
STATUT_SCAN_VALIDE = "VALIDE"
STATUT_SCAN_REJETE = "REJETE"
STATUT_SCAN_ERREUR = "ERREUR"

# Colonnes légères : la grille de supervision ne charge jamais le PDF.
_COLONNES_SCAN = (
    "id, message_id, source, expediteur, destinataire, objet, nom_fichier, "
    "taille_octets, nb_pages, sens, recu_le, ingere_le, statut, confiance, "
    "motif_validation, id_bl, verrouille_par, verrouille_le, analyse_le, "
    "traite_le, traite_par, tentatives, erreur"
)


def enregistrer_scan(message_id: str, source: str, expediteur: str,
                     destinataire: str, objet: str, nom_fichier: str,
                     contenu: bytes, nb_pages: Optional[int], sens: str,
                     recu_le: datetime.datetime) -> Optional[int]:
    """Dépose un scan dans la file. Renvoie son id, ou None si déjà présent.

    `message_id` est UNIQUE : c'est l'idempotence de tout le pipeline. Un mail
    livré deux fois, une relève rejouée ou un job relancé ne produisent qu'une
    seule ligne — et le None permet à l'appelant de le journaliser sans en
    faire une erreur."""
    s = get_settings()
    with database.transaction() as tx:
        ligne = tx.fetch_one(
            f"INSERT INTO {s.pg_schema}.scans_recus "
            "(message_id, source, expediteur, destinataire, objet, nom_fichier, "
            " contenu, taille_octets, nb_pages, sens, recu_le) "
            "VALUES (%(mid)s, %(src)s, %(exp)s, %(dest)s, %(obj)s, %(nom)s, "
            "        %(c)s, %(taille)s, %(pages)s, %(sens)s, %(recu)s) "
            "ON CONFLICT (message_id) DO NOTHING RETURNING id",
            {"mid": message_id, "src": source, "exp": expediteur or None,
             "dest": destinataire or None, "obj": objet or None,
             "nom": nom_fichier, "c": contenu, "taille": len(contenu),
             "pages": nb_pages, "sens": sens, "recu": recu_le})
    return int(ligne["id"]) if ligne else None


def prendre_scans_a_analyser(limite: int = 20) -> list[dict]:
    """Réserve des scans pour analyse et renvoie leur PDF.

    `FOR UPDATE SKIP LOCKED` puis passage en EN_ANALYSE dans la même
    transaction : deux exécutions simultanées du job ne traitent jamais le
    même scan, et un job interrompu laisse le scan en EN_ANALYSE — repris
    ensuite par la maintenance."""
    s = get_settings()
    with database.transaction() as tx:
        candidats = tx.fetch_dataframe(
            f"SELECT id FROM {s.pg_schema}.scans_recus "
            "WHERE statut = 'RECU' ORDER BY recu_le LIMIT %(lim)s "
            "FOR UPDATE SKIP LOCKED", {"lim": limite})
        if candidats is None or candidats.empty:
            return []
        ids = [int(x) for x in candidats["id"].tolist()]
        tx.execute(
            f"UPDATE {s.pg_schema}.scans_recus "
            "SET statut = 'EN_ANALYSE', analyse_le = now(), "
            "    tentatives = tentatives + 1 "
            "WHERE id = ANY(%(ids)s)", {"ids": ids})
        lignes = tx.fetch_dataframe(
            f"SELECT id, contenu, sens, nb_pages, recu_le, nom_fichier, "
            f"expediteur FROM {s.pg_schema}.scans_recus "
            "WHERE id = ANY(%(ids)s) ORDER BY recu_le", {"ids": ids})
    return [{**ligne, "contenu": bytes(ligne["contenu"])}
            for ligne in lignes.to_dict("records")]


def cloturer_scan(scan_id: int, statut: str, *, confiance: str = "",
                  motif: str = "", extraction_json: Optional[dict] = None,
                  id_bl: Optional[str] = None, nb_pages: Optional[int] = None,
                  traite_par: str = "", erreur: str = "") -> None:
    """Fin de traitement d'un scan : statut, verdict et traces."""
    s = get_settings()
    _run(
        f"UPDATE {s.pg_schema}.scans_recus SET statut = %(st)s, "
        "confiance = %(conf)s, motif_validation = %(motif)s, "
        "extraction = COALESCE(%(ex)s::jsonb, extraction), "
        "id_bl = COALESCE(%(bl)s, id_bl), "
        "nb_pages = COALESCE(%(pages)s, nb_pages), "
        "traite_le = CASE WHEN %(st)s IN ('TRAITE_AUTO', 'VALIDE', 'REJETE') "
        "                 THEN now() ELSE traite_le END, "
        "traite_par = COALESCE(NULLIF(%(par)s, ''), traite_par), "
        "erreur = NULLIF(%(err)s, ''), "
        "verrouille_par = NULL, verrouille_le = NULL "
        "WHERE id = %(id)s",
        params={"id": scan_id, "st": statut, "conf": confiance or None,
                "motif": motif or None,
                "ex": json.dumps(extraction_json, ensure_ascii=False, default=str)
                      if extraction_json is not None else None,
                "bl": id_bl, "pages": nb_pages, "par": traite_par,
                "err": erreur})


def lister_scans(statuts: Optional[list[str]] = None, sens: str = "",
                 date_min: Optional[datetime.date] = None,
                 date_max: Optional[datetime.date] = None,
                 limite: int = 300) -> pd.DataFrame:
    """Catalogue des scans, **sans le PDF** (colonne lourde chargée à la
    demande par `telecharger_scan`)."""
    s = get_settings()
    conditions, params = ["true"], {"lim": limite}
    if statuts:
        conditions.append("statut = ANY(%(st)s)")
        params["st"] = list(statuts)
    if sens:
        conditions.append("sens = %(sens)s")
        params["sens"] = sens
    if date_min:
        conditions.append("recu_le >= %(dmin)s")
        params["dmin"] = date_min
    if date_max:
        conditions.append("recu_le < %(dmax)s")
        params["dmax"] = date_max + datetime.timedelta(days=1)
    return _run(
        f"SELECT {_COLONNES_SCAN}, extraction "
        f"FROM {s.pg_schema}.scans_recus WHERE {' AND '.join(conditions)} "
        "ORDER BY recu_le DESC LIMIT %(lim)s", params=params, fetch=True)


def telecharger_scan(scan_id: int) -> Optional[dict]:
    """PDF et métadonnées d'un scan, ou None s'il n'existe plus."""
    s = get_settings()
    with database.transaction() as tx:
        ligne = tx.fetch_one(
            f"SELECT {_COLONNES_SCAN}, extraction, contenu "
            f"FROM {s.pg_schema}.scans_recus WHERE id = %(id)s", {"id": scan_id})
    if ligne is None:
        return None
    ligne["contenu"] = bytes(ligne["contenu"])
    return ligne
...
def verrouiller_scan(scan_id: int, utilisateur: str,
                     expiration_minutes: int = 60) -> bool:
    """Prise en charge exclusive d'un scan à valider.

    Renvoie False si un autre gestionnaire l'a déjà pris et que son verrou
    n'a pas expiré — l'appelant affiche alors un message plutôt que d'ouvrir
    un écran de validation concurrent."""
    s = get_settings()
    with database.transaction() as tx:
        touchees = tx.execute(
            f"UPDATE {s.pg_schema}.scans_recus "
            "SET verrouille_par = %(u)s, verrouille_le = now() "
            "WHERE id = %(id)s AND statut = 'A_VALIDER' "
            "  AND (verrouille_par IS NULL OR verrouille_par = %(u)s "
            "       OR verrouille_le < now() - make_interval(mins => %(exp)s))",
            {"id": scan_id, "u": utilisateur, "exp": expiration_minutes})
    return touchees > 0


def liberer_scan(scan_id: int) -> None:
    s = get_settings()
    _run(f"UPDATE {s.pg_schema}.scans_recus "
         "SET verrouille_par = NULL, verrouille_le = NULL WHERE id = %(id)s",
         params={"id": scan_id})


def purger_contenu_scans(jours: int) -> int:
    """Libère le PDF des scans terminés depuis plus de `jours`.

    La ligne (traces, extraction, lien vers le BL) est CONSERVÉE : seule la
    colonne lourde est vidée. Les images des pages restent attachées au BL."""
    s = get_settings()
    with database.transaction() as tx:
        return tx.execute(
            f"UPDATE {s.pg_schema}.scans_recus SET contenu = ''::bytea, "
            "taille_octets = 1 "
            "WHERE statut IN ('TRAITE_AUTO', 'VALIDE', 'REJETE') "
            "  AND traite_le < now() - make_interval(days => %(j)s) "
            "  AND length(contenu) > 1", {"j": jours})


def stats_scans(depuis: datetime.date) -> dict:
    """Compteurs par statut depuis une date — supervision du pipeline."""
    s = get_settings()
    df = _run(
        f"SELECT statut, count(*) AS n FROM {s.pg_schema}.scans_recus "
        "WHERE recu_le >= %(d)s GROUP BY statut", params={"d": depuis},
        fetch=True)
    if df is None or df.empty:
        return {}
    return {str(ligne["statut"]): int(ligne["n"]) for _, ligne in df.iterrows()}


def dernier_scan_recu() -> Optional[datetime.datetime]:
    """Horodatage du dernier scan ingéré — sert à détecter un pipeline muet,
    panne bien plus dangereuse qu'une erreur bruyante."""
    s = get_settings()
    df = _run(f"SELECT max(recu_le) AS dernier FROM {s.pg_schema}.scans_recus",
              fetch=True)
    if df is None or df.empty or pd.isna(df["dernier"].iloc[0]):
        return None
    return df["dernier"].iloc[0]


def liberer_scans_abandonnes(heures: int) -> int:
    """Remet en file les scans bloqués : verrou de validation oublié, ou
    analyse interrompue par un job tué en cours d'exécution."""
    s = get_settings()
    with database.transaction() as tx:
        verrous = tx.execute(
            f"UPDATE {s.pg_schema}.scans_recus "
            ...
            "  AND verrouille_le < now() - make_interval(hours => %(h)s)",
            {"h": heures})
        analyses = tx.execute(
            f"UPDATE {s.pg_schema}.scans_recus SET statut = 'RECU' "
            "WHERE statut = 'EN_ANALYSE' "
            "  AND analyse_le < now() - make_interval(hours => %(h)s)",
            {"h": heures})
    return verrous + analyses
...
def creer_bl_depuis_scan(id_bl: str, numero_bl: str, nom_fournisseur: str,
                         statut_bl: str, type_operation: str, utilisateur: str,
                         pages: list[bytes], origine: str,
                         date_reception: Optional[datetime.date] = None,
                         quai_reception: Optional[str] = None,
                         comment_bl: str = "",
                         plage_horaire: Optional[str] = None) -> None:
    """Crée un BL COMPLET avec toutes ses pages, **en une seule transaction**.

    Contrairement à la saisie interactive (insertion, puis pages, puis
    finalisation — chaque étape reprenable si l'opérateur revient), un
    traitement automatisé n'a personne pour reprendre la main : soit le BL
    existe entièrement, soit il n'existe pas du tout. Un BROUILLON sans pages
    laissé par un job interrompu polluerait durablement les indicateurs.

    `origine` ∈ SCAN_AUTO (créé par le pipeline) | SCAN_VALIDE (après
    validation humaine) | MANUEL. Lève ValueError si le numéro existe déjà.
    """
    s = get_settings()
    numero_bl = normalize_bl_number(numero_bl)
    if not pages:
        raise ValueError("Un BL doit porter au moins une page scannée.")
    try:
        with database.transaction() as tx:
            tx.execute(
                f"""
                INSERT INTO {s.pg_schema}.suivi_bl
                  (id_bl, numero_bl, date_reception, plage_horaire, nom_fournisseur,
                   ...
                   type_operation, est_supprime, source_donnee, document_statut,
                   origine, version)
                VALUES
                  (%(id)s, %(num)s, %(dr)s, %(plage)s, %(frs)s, %(quai)s,
                   %(st)s, %(com)s, %(par)s, now(), %(op)s, false, 'MANUEL',
                   'COMPLET', %(orig)s, 1)
                """,
                {"id": id_bl, "num": numero_bl, "dr": date_reception,
                 "plage": plage_horaire, "frs": nom_fournisseur,
                 "quai": quai_reception, "st": statut_bl, "com": comment_bl,
                 "par": utilisateur, "op": type_operation, "orig": origine})
            for index, image in enumerate(pages):
                if len(image) > s.max_image_bytes:
                    raise ValueError(
                        f"La page {index + 1} dépasse la taille autorisée "
                        f"({len(image)} > {s.max_image_bytes} octets).")
                tx.execute(
                    f"INSERT INTO {s.pg_schema}.pieces_jointes_bl "
                    "(id_photo, id_bl, contenu, sha256, taille_octets, "
                    " content_type, index_page) "
                    "VALUES (%(idp)s, %(idb)s, %(c)s, %(sha)s, %(taille)s, "
                    "        'image/jpeg', %(idx)s)",
                    {"idp": str(uuid.uuid4()), "idb": id_bl, "c": image,
                     "sha": hashlib.sha256(image).hexdigest(),
                     "taille": len(image), "idx": index})
            _auditer_tx(tx, id_bl, "CREATION", utilisateur, apres=numero_bl)
            _auditer_tx(tx, id_bl, f"ORIGINE_{origine}", utilisateur,
                        apres=f"{len(pages)} page(s)")
    except psycopg.errors.UniqueViolation:
...
def _conditions_bl(numero, fournisseur, types, date_min, date_max, statut,
                   gestionnaire, inclure_supprimes, prefixe: str = "",
                   champ_date: str = CHAMP_DATE_OPERATION) -> tuple[str, dict]:
    ...
    `champ_date` choisit la colonne visée par [date_min, date_max] :
    `date_reception` (date métier) ou `saisie_le` (date d'enregistrement — utile
    pour retrouver un archivage saisi hier sur un BL de 2024)."""
    s = get_settings()
    p = prefixe
    ...
    if numero:
        ...
        params["num"] = f"%{numero.lower()}%"
    if champ_date not in CHAMPS_DATE:
        raise ValueError(f"champ_date invalide : {champ_date!r}.")
    if champ_date == CHAMP_DATE_SAISIE:
        # saisie_le est un TIMESTAMPTZ : la borne haute doit couvrir la
        # journée entière, d'où « < lendemain » plutôt que « <= jour ».
        if date_min:
            conditions.append(f"{p}saisie_le >= %(dmin)s")
            params["dmin"] = date_min
        if date_max:
            conditions.append(f"{p}saisie_le < %(dmax)s")
            params["dmax"] = date_max + datetime.timedelta(days=1)
    else:
        if date_min:
...
def stats_bl(numero="", fournisseur="", types=None, date_min=None, date_max=None,
             gestionnaire="", inclure_supprimes=False,
             champ_date: str = CHAMP_DATE_OPERATION) -> dict:
...
def rechercher_bl(
    ...
    page: int = 1,
    page_size: int = 50,
    champ_date: str = CHAMP_DATE_OPERATION,
    ...
    `champ_date` choisit la colonne visée par la période (date d'opération ou
    date de saisie)."""
    s = get_settings()
    where, params = _conditions_bl(numero, fournisseur, types, date_min, date_max,
