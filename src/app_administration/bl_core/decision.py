...
def sens_du_destinataire(textes: list[str], alias_achat: str,
                         alias_vente: str) -> Optional[str]:
    """ACHAT / VENTE déduit de l'alias visé par le mail.
    ...
    « BL Expédition » sur l'écran du copieur.

    `textes` n'est pas seulement la liste des destinataires : Exchange Online
    **résout l'alias vers l'adresse principale** de la boîte dans le champ
    destinataire, si bien qu'un mail envoyé à `scans-bl-reception@…` y apparaît
    comme `scans-bl@…`. Le sens serait alors perdu. On cherche donc le fragment
    dans un ensemble plus large — destinataires **plus** sujet **plus** en-têtes
    de routage (voir `graph_mail.texte_routage`), où le destinataire d'enveloppe
    survit.

    None si aucun fragment ne ressort, **ou si les deux apparaissent** (chaîne
    de transfert, mail ambigu) : le scan est écarté plutôt que rangé au hasard,
    une erreur de sens étant coûteuse à rattraper."""
    blob = " ".join((t or "").lower() for t in textes)
    achat = bool(alias_achat) and alias_achat.lower() in blob
    vente = bool(alias_vente) and alias_vente.lower() in blob
    if achat and not vente:
        return "ACHAT"
    if vente and not achat:
        return "VENTE"
    return None
