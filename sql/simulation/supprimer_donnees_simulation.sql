...
DELETE FROM bl_scan.pieces_jointes_bl WHERE id_bl LIKE 'SIM-%';
DELETE FROM bl_scan.audit_bl          WHERE id_bl LIKE 'SIM-%';
DELETE FROM bl_scan.suivi_bl          WHERE id_bl LIKE 'SIM-%';
DELETE FROM bl_scan.base_desadv       WHERE numero_bl LIKE 'SIM-%';
DELETE FROM bl_scan.notifications     WHERE event_key LIKE 'SIM-%';
DELETE FROM bl_scan.qualite_extraction WHERE numero_bl LIKE 'SIM-%';
DELETE FROM bl_scan.portefeuilles
  WHERE code_gestionnaire LIKE 'SIM-%' OR nom_fournisseur LIKE 'SIM-%';
DELETE FROM bl_scan.pla               WHERE nom_fournisseur LIKE 'SIM-%';
DELETE FROM bl_scan.sites_logistiques
  WHERE entite LIKE 'SIM-%' OR adresse LIKE 'SIM-%';
DELETE FROM bl_scan.adresses          WHERE adresse LIKE 'SIM-%';
DELETE FROM bl_scan.gestionnaires     WHERE code_gestionnaire LIKE 'SIM-%';
DELETE FROM bl_scan.base_tiers        WHERE name LIKE 'SIM-%';
...
-- DELETE FROM bl_scan.rapports_activite;

COMMIT;

-- Contrôle : doit renvoyer 0 partout.
SELECT 'suivi_bl' AS table_cible, count(*) AS restant FROM bl_scan.suivi_bl WHERE id_bl LIKE 'SIM-%'
UNION ALL SELECT 'base_tiers', count(*) FROM bl_scan.base_tiers WHERE name LIKE 'SIM-%'
UNION ALL SELECT 'base_desadv', count(*) FROM bl_scan.base_desadv WHERE numero_bl LIKE 'SIM-%'
UNION ALL SELECT 'gestionnaires', count(*) FROM bl_scan.gestionnaires WHERE code_gestionnaire LIKE 'SIM-%'
UNION ALL SELECT 'adresses', count(*) FROM bl_scan.adresses WHERE adresse LIKE 'SIM-%';
