...
`bl_core` ne dépend pas de Streamlit : `cache.py` bascule automatiquement sur
`functools.lru_cache` hors application. Aucune logique métier n'est dupliquée
entre l'app et les jobs — c'est **le même** `decision.py` et le même
`extraction.py`.
...
| Le mauvais sens (achat/vente) | L'alias destinataire est la seule source. Vérifier la configuration du bouton du copieur, puis `alias_achat`/`alias_vente`. Un mail sans alias reconnu est **rejeté**, jamais rangé au hasard. |
