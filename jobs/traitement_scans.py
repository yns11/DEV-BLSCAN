...
def traiter_un_scan(scan: dict, modules, dpi: int) -> str:
    ...
    decision, extraction, notifications, repository, scan_pdf, ui = modules
    scan_id = int(scan["id"])
    sens = scan["sens"]
    ...
    if type_operation == repository.TYPE_RECEPTION:
        succes, detail = notifications.notifier_nouvelle_reception(
            ...
            plage_horaire=champs["plage_horaire"],
            statut_libelle=ui.libelle_statut(champs["statut_bl"]),
...
def main() -> None:
    ...
    modules = (decision, extraction, notifications, repository, scan_pdf, ui)
