...
def main() -> None:
    ...
    verifier_bl_core("decision", "graph_mail", "repository", "scan_pdf")
    ...
    for message in boite.messages_non_lus(limite=int(args.max_messages)):
        ...
        # --- Sens du flux -------------------------------------------------
        # Exchange résout l'alias vers l'adresse principale dans le champ
        # destinataire : on cherche donc le fragment aussi dans le sujet et
        # dans les en-têtes de routage, où le destinataire d'enveloppe survit.
        entetes = boite.texte_routage(message.id)
        signaux = [*message.destinataires, message.objet, entetes]
        sens = decision.sens_du_destinataire(
            signaux, args.alias_achat, args.alias_vente)
        if sens is None:
            vus = ", ".join(message.destinataires) or "(aucun)"
            rejets.append(
                f"sens indéterminé — destinataires vus : [{vus}], "
                f"sujet : « {message.objet} », en-têtes de routage : "
                f"{'oui' if entetes else 'illisibles'} "
                f"(alias attendus : « {args.alias_achat} » / "
                f"« {args.alias_vente} »)")
